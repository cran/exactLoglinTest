...
\begin{document} 
\maketitle 
Note, a more detailed, though static, description of the software is
available in \cite{caffo2006exact}. To reference \texttt{exactLoglinTest},
please use:
\noindent
\begin{verbatim}
...
\texttt{exactLoglinTest} and considered a useful extension of the
program to disclorsure limitation.  The program was initially
constructed calculate P-values for goodness of fit tests for
contingency tables.

In previous versions of this vignette, we discussed using
\texttt{exactLoglinTest} for performing tests in exact binomial
logistic regression.  It is true that conditional Poisson loglinear
models contain conditional logistic regression models as special
cases.  However, we have found that in some cases, the matrices used
by \texttt{exactLoglinTest} for this purpose are ill conditioned. Thus
we no longer recommend \texttt{exactLoglinTest} for pursuing
conditional logistic regression models, unless the user is particularly
familiar with the area. The paper \cite{caffo2006exact} gives more
details on exact binomial testing.
